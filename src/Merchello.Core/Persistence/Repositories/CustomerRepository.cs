namespace Merchello.Core.Persistence.Repositories
{
	...
	internal class CustomerRepository : PagedRepositoryBase<ICustomer, CustomerDto>, ICustomerRepository
	{
		...
		/// <summary>
		/// Adds a customer to a static customer collection.
		/// </summary>
		/// <param name="entityKey">
		/// The entity key.
		/// </param>
		...
		{
			AddToCollection(entityKey, collectionKey, null);
		}
		...
		public void AddToCollection(Guid entityKey, Guid collectionKey, int? sortOrder = null)
		{
			...
			var sql = new Sql();
			sql.Append("SELECT * FROM [merchCustomer2EntityCollection]")
				.Append(
					"WHERE [merchCustomer2EntityCollection].[customerKey] = @ekey AND [merchCustomer2EntityCollection].[entityCollectionKey] = @eckey",
					new { @ekey = entityKey, @eckey = collectionKey });

			Customer2EntityCollectionDto dto = Database.FirstOrDefault<Customer2EntityCollectionDto>(sql);

			// todo reorder products in this collection
			if (dto == null)
			{
				dto = new Customer2EntityCollectionDto()
				{
			...
			else if (sortOrder.HasValue)
			{
				if (dto.SortOrder != sortOrder.Value)
				{
					...
					Database.Update<Customer2EntityCollectionDto>("SET SortOrder=@0 WHERE CustomerKey=@1 AND entityCollectionKey=@2", dto.SortOrder, dto.CustomerKey, dto.EntityCollectionKey);
				}
			}
		}
			...
			Action<Sql> fnModifySql = null)
		{
			...
			else
			{
				...
				{
					orderExpression = "loginName"; // default
				}
			}

			sql.Append("FROM [merchCustomer] [A]");

			if (sortCollectionGuid != Guid.Empty)
			{
				sql.Append(" LEFT JOIN [merchCustomer2EntityCollection] B ON [A].[customerKey]=[B].[customerKey] AND [B].[entityCollectionKey] = @eck", new { @eck = sortCollectionGuid });
			}
		...
		/// </param>
		/// <param name="sortCollectionGuid">
		/// The guid of the collection in which can be sorted
		/// </param>
		/// <param name="fnModifySql">
		/// Delegate function, purpose is to modify the sql
		/// </param>
			...
			Action<Sql> fnModifySql = null)
		{
			...
			var invidualTerms = term.Split(' ');
			var terms = invidualTerms.Where(x => !string.IsNullOrEmpty(x)).ToList();					
			if (terms.Any())
			{
				var preparedTerms = string.Format("%{0}%", string.Join("% ", terms)).Trim();

				sql.Where("[A].lastName LIKE @ln OR [A].firstName LIKE @fn OR [A].email LIKE @email", new { @ln = preparedTerms, @fn = preparedTerms, @email = preparedTerms });
			}
		...
		public Page<ICustomer> GetEntitiesThatExistInAllCollections(
			...
			SortDirection sortDirection = SortDirection.Descending)
		{
			return GetEntitiesThatExistInAllCollections(collectionKeys, page, itemsPerPage, orderExpression, sortDirection, Guid.Empty);
		}
		...
		public Page<ICustomer> GetEntitiesThatExistInAllCollections(
			...
			SortDirection sortDirection = SortDirection.Descending)
		{
			return GetEntitiesThatExistInAllCollections(collectionKeys, term, page, itemsPerPage, orderExpression, sortDirection, Guid.Empty);
		}
		...
		public Page<ICustomer> GetEntitiesThatExistInAllCollections(
			...
			Guid sortCollectionGuid = new Guid())
		{
			var p = this.GetKeysThatExistInAllCollections(collectionKeys, term, page, itemsPerPage, orderExpression, sortDirection, sortCollectionGuid);
			return new Page<ICustomer>()
			{
				CurrentPage = p.CurrentPage,
				ItemsPerPage = p.ItemsPerPage,
