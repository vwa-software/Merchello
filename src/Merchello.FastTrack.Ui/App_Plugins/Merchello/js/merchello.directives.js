	...
	angular.module('merchello.directives').directive('entityStaticCollections',
		function () {
			return {
				...
				link: function (scope, element, attrs) {
					element.on('focusout', 'input.sortOrder', function () {
						console.log('foucusout');
						scope.blur(angular.element(this).scope());
					});
				},
			...
			function ($routeParams, $log, $filter, $compile, dialogService, eventsService, localizationService, merchelloListViewHelper, queryDisplayBuilder, queryResultDisplayBuilder) {
				return {
					...
					link: function (scope, elm, attr) {
						...
						scope.config = angular.copy(merchelloListViewHelper.getConfig(scope.entityType));
						...
						function init() {
							...
							if (scope.collectionKey == '') {
								var index = -1;
								angular.forEach(scope.config.columns, function (value, key) {
									if (value.name == 'sortOrder') {
										index = key;
									}
								});
								if (index >= 0) {
