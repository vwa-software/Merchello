namespace Merchello.Core.Persistence.Repositories
{
	...
	internal partial class ProductRepository : PagedRepositoryBase<IProduct, ProductDto>, IProductRepository
	{
		...
		public void AddToCollection(Guid entityKey, Guid collectionKey, int? sortOrder = null)
		{
			...
			var sql = new Sql();
			sql.Append("SELECT * FROM [merchProduct2EntityCollection]")
			   .Append("WHERE [merchProduct2EntityCollection].[productKey] = @pkey AND [merchProduct2EntityCollection].[entityCollectionKey] = @eckey",
					new { @pkey = entityKey, @eckey = collectionKey });

			Product2EntityCollectionDto dto = Database.FirstOrDefault<Product2EntityCollectionDto>(sql);
			
			if (dto == null)
			{
				dto = new Product2EntityCollectionDto()
				{
			...
			else if (sortOrder.HasValue)
			{
				if (dto.SortOrder != sortOrder.Value)
				{
					...
					Database.Update<Product2EntityCollectionDto>("SET SortOrder=@0 WHERE productKey=@1 AND entityCollectionKey=@2", dto.SortOrder, dto.ProductKey, dto.EntityCollectionKey);
				}
			}
		}
			...
			Action<Sql> fnModifySql = null)
		{
			...
			sql.Where("[A].[master] = @master", new { @master = true })
			 .Append("AND [A].[master] = 1");
